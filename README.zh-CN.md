...
| [codex-external-subagent-bridge](codex-external-subagent-bridge/) | 仅面向 Codex Desktop 的启动器与路由契约 Bridge。项目置顶入口只接受精确小写 `new`，以 GPT-5.6 Luna 创建零 bootstrap turn 的 V1 任务，再无 turn 切换到 GPT-5.6 Sol Ultra；仅暴露已启用、本机冒烟通过且配置指纹一致的现有 child/MCP 路线。Provider 由用户自行定义，Skill 不修改 Provider、MCP、Keychain、CC Switch 或全局模型配置。 |
