...
| [gpt-subagent-external-router](gpt-subagent-external-router/) | Codex 官方 Multi-Agent V2 无法可靠满足“将外部模型定义为子 Agent”的需求；一次配置本 Skill 后，每个获批项目使用一个置顶 Codex 入口，用户只需在入口中输入精确的“新建”。入口将任务锁定为 V1，以 GPT-5.6 Sol 作为主 Agent，并提供通过已完成烟雾测试的路由交付外部子任务的契约。启动器本身不实现供应商调度。作者目前仅实测 DeepSeek、通过 CC Switch 接入的 Kimi 及 Grok CLI 工具；其他供应商均未实测。 |
