...
| [gpt-subagent-external-router](gpt-subagent-external-router/) | Codex's official Multi-Agent V2 cannot reliably satisfy the requirement of defining external models as subagents; configure this Skill once, then use one project-pinned Codex entry per approved project where the user types only `新建`. The entry locks the task to V1, uses GPT-5.6 Sol as root, and provides a contract for routing external subtasks through smoke-tested routes. The launcher does not implement provider dispatch. The author's tested routes currently cover DeepSeek, Kimi through CC Switch, and a Grok CLI tool; other providers remain untested. |
